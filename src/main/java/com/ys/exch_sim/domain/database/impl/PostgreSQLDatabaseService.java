...
/** PostgreSQL実装のDatabaseService Spring Data JPAを使用してデータベース操作を実装 */
@Slf4j
@Service
...
public class PostgreSQLDatabaseService implements DatabaseService {
  ...
  public void insertExecution(Execution execution) {
    try {
      ...
      log.info(
          "Successfully inserted execution to PostgreSQL: execID={}, username={}, symbol={},"
              + " qty={}",
          execution.getExecID(),
  ...
  public List<Execution> queryRecentExecutions(LocalDateTime fromTime) {
    try {
      ...
      return allExecutions.stream()
          .filter(
              exec ->
                  exec.getCreatedAt() != null
                      && (exec.getCreatedAt().isAfter(fromTime)
                          || exec.getCreatedAt().isEqual(fromTime)))
  ...
  public void upsertPosition(Position position) {
    try {
      ...
      log.debug(
          "Successfully upserted position to PostgreSQL: {}_{}",
          position.getUsername(),
          position.getSymbol());
    } catch (Exception e) {
      log.error(
          "Error upserting position to PostgreSQL: {}_{}",
          position.getUsername(),
          position.getSymbol(),
          e);
  ...
  public Position queryPosition(String username, String symbol) {
    try {
      return positionRepository
          .findByUsernameAndSymbol(username, symbol.toUpperCase())
          .map(PositionEntity::toPosition)
          .orElse(null);
    } catch (Exception e) {
      log.error(
          "Error querying position from PostgreSQL for user: {} symbol: {}", username, symbol, e);
      return null;
    }
  }
  ...
  public List<Position> queryAllPositions(String username) {
    try {
      ...
      return entities.stream().map(PositionEntity::toPosition).collect(Collectors.toList());
    } catch (Exception e) {
  ...
  public List<TradeHistory> queryTradeHistory(String username) {
    try {
      List<TradeHistoryEntity> entities =
          tradeHistoryRepository.findByUsernameOrderByTimestampDesc(username);
      return entities.stream().map(TradeHistoryEntity::toTradeHistory).collect(Collectors.toList());
    } catch (Exception e) {
  ...
  public List<TradeHistory> queryTradeHistory(String username, String symbol) {
    try {
      List<TradeHistoryEntity> entities =
          tradeHistoryRepository.findByUsernameAndSymbolOrderByTimestampDesc(
              username, symbol.toUpperCase());
      return entities.stream().map(TradeHistoryEntity::toTradeHistory).collect(Collectors.toList());
    } catch (Exception e) {
      log.error(
          "Error querying trade history from PostgreSQL for user: {} symbol: {}",
          username,
          symbol,
          e);
      return List.of();
  ...
  public List<TradeHistory> queryTradeHistory(String username, int limit) {
    try {
      ...
      log.error(
          "Error querying trade history from PostgreSQL for user: {} with limit: {}",
          username,
          limit,
          e);
      return List.of();
  ...
  public DatabaseService.UserEntity loadUser(String username) {
    try {
      return userRepository
          ...
          .map(
              user ->
                  new DatabaseService.UserEntity(
